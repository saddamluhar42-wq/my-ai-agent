...
def _you_search(prompt):
    ...
    for key in keys:
        try:
            req = urllib.request.Request(f"https://ydc-index.io/v1/search?query={query}&count=5", headers={"X-API-Key": key, "Accept": "application/json"})
            with urllib.request.urlopen(req, timeout=30) as response: data = json.loads(response.read().decode())
            results = data.get("results", {}).get("web", [])
            if not results: raise AgentError("You.com: no results returned.")
            answer = "\n".join(f"- {item.get('title', 'Result')}: {item.get('description', '')}" for item in results[:5])
            return {"answer": answer, "provider": "You.com", "model": YOU_MODEL or "Web Search", "type": "text"}
        except Exception as exc: last_error = exc
...
def generate_image(prompt):
    ...
    except Exception as exc: raise AgentError(f"Hugging Face image generation failed: {exc}") from exc
...
def provider_status(): return {name: name in get_available_providers() for name in TEXT_PRIORITY} | {"Hugging Face Image": huggingface.is_configured()}
