...
from config import (
    ...
    YOU_API_KEY, YOU_API_KEY_2, YOU_API_KEY_3, YOU_MODEL,
)
...
def _you_search(prompt):
    ...
    for key in keys:
        try:
            request = urllib.request.Request(
                f"https://ydc-index.io/v1/search?query={query}&count=5",
...
def get_available_providers():
    available = []
    if gemini.is_configured(): available.append("Gemini")
    if DEEPSEEK_API_KEY or DEEPSEEK_API_KEY_2 or DEEPSEEK_API_KEY_3: available.append("DeepSeek")
    if anthropic.is_configured(): available.append("Anthropic")
    if KIMI_API_KEY or KIMI_API_KEY_2 or KIMI_API_KEY_3: available.append("Kimi")
    if OPENAI_API_KEY or OPENAI_API_KEY_2 or OPENAI_API_KEY_3: available.append("OpenAI")
    if XAI_API_KEY or XAI_API_KEY_2 or XAI_API_KEY_3: available.append("xAI")
    if openrouter.is_configured(): available.append("OpenRouter")
    if YOU_API_KEY or YOU_API_KEY_2 or YOU_API_KEY_3: available.append("You.com")
    return available
    ...
    for provider in providers:
        try:
            ...
            if provider == "DeepSeek":
                return _openai_compatible("DeepSeek", [DEEPSEEK_API_KEY, DEEPSEEK_API_KEY_2, DEEPSEEK_API_KEY_3], "https://api.deepseek.com/chat/completions", DEEPSEEK_MODEL, prompt, temperature, max_tokens)
            ...
            if provider == "Kimi":
                return _openai_compatible("Kimi", [KIMI_API_KEY, KIMI_API_KEY_2, KIMI_API_KEY_3], "https://api.moonshot.ai/v1/chat/completions", KIMI_MODEL, prompt, temperature, max_tokens)
            if provider == "OpenAI":
                return _openai_compatible("OpenAI", [OPENAI_API_KEY, OPENAI_API_KEY_2, OPENAI_API_KEY_3], "https://api.openai.com/v1/chat/completions", OPENAI_MODEL, prompt, temperature, max_tokens)
            if provider == "xAI":
                return _openai_compatible("xAI", [XAI_API_KEY, XAI_API_KEY_2, XAI_API_KEY_3], "https://api.x.ai/v1/chat/completions", XAI_MODEL, prompt, temperature, max_tokens)
