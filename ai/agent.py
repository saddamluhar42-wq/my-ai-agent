...
def _needs_web_search(prompt):
    ...
    if text in simple:
        return False
    triggers = (
        "today", "latest", "current", "now", "recent", "2026", "price", "rate", "weather", "news", "update",
        "who is", "when did", "how much", "compare", "research", "history", "origin", "source", "official",
        "rule", "law", "legal", "policy", "market", "stock", "gold", "silver", "bitcoin", "ai model", "version",
        "ranking", "best", "top", "schedule", "release", "announcement", "internet", "web",
    )
...
def research_web(prompt, deep=False):
    ...
    return {
        ...
        "tavily_key_index": result.get("tavily_key_index"),
    }
...
def generate(prompt, preferred_provider=None, temperature=None, max_tokens=None):
    ...
    for provider in providers:
        try:
            ...
            if research_meta:
                response["research"] = {
                    "providers": research_meta["providers"],
                    "result_count": research_meta["result_count"],
                    "errors": research_meta.get("errors", []),
                    "primary": research_meta.get("primary"),
                    "fallback_used": research_meta.get("fallback_used", False),
                    "tavily_key_index": research_meta.get("tavily_key_index"),
                }
            return response
        except Exception as exc:
...
def provider_status():
    return {
        name: name in get_available_providers() for name in TEXT_PRIORITY
    } | {
        ...
        "Tavily Key Count": len(_keys(TAVILY_API_KEY, TAVILY_API_KEY_2, TAVILY_API_KEY_3)),
