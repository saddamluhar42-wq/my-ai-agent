...
def _get_message_text(message: dict) -> str:
    return _clean_text(message.get("text") or message.get("caption"))
...
def _get_user_id(message: dict) -> Optional[int]:
    ...
    try:
        ...
        display_name = (f"{user.get('first_name', '')} {user.get('last_name', '')}").strip() or user.get("username") or "Telegram User"
        return get_or_create_user(external_id=f"telegram:{telegram_id}", display_name=display_name)
    except Exception:
        return None
...
def _is_time_question(text: str) -> bool:
    ...
    question_phrases = ("what time", "tell me the time", "time kya", "time bata", "kitne baje hai", "kitna time hai", "abhi time", "current time", "local time", "samay kya", "samay bata", "waqt kya", "waqt bata")
...
def _looks_like_task(text: str) -> bool:
    ...
    if not normalized or _is_time_question(normalized):
        return False
    if "?" in normalized and not any(marker in normalized for marker in ("remind", "yaad", "task")):
        return False
...
def _extract_due_at(text: str, timezone_name: str = DEFAULT_TIMEZONE) -> Optional[datetime]:
    ...
    elif not any(marker in normalized for marker in _DATE_TODAY):
        candidate_today = datetime(target_date.year, target_date.month, target_date.day, hour, minute, tzinfo=tz)
        if candidate_today <= now:
            target_date += timedelta(days=1)
    return datetime(target_date.year, target_date.month, target_date.day, hour, minute, tzinfo=tz)


def parse_scheduled_task(text: str, timezone_name: str = DEFAULT_TIMEZONE) -> Optional[Tuple[str, datetime]]:
    clean = _clean_text(text)
    if not clean or not _looks_like_task(clean):
        return None
    due_at = _extract_due_at(clean, timezone_name=timezone_name)
    return (clean, due_at) if due_at is not None else None
...
def _handle_task_command(message: dict, text: str) -> bool:
    ...
    if normalized == "/tasks" or normalized.startswith("/tasks "):
        ...
        for row in rows:
            ...
            lines.append(f"#{row['id']} • {row['status']} • {due}\n{row['task_text']}")
    ...
    if normalized.startswith("/cancel_task"):
        ...
        try:
            ...
            return True
        send_message(chat_id, f"{'✅ Task cancelled.' if cancelled else 'Task nahi mila ya already execute ho chuka hai.'} #{task_id}")
        return True
    return False
...
def try_schedule_message(message: dict) -> bool:
    ...
    if not text:
        return False
    chat_id = _get_chat_id(message)
    if chat_id and DATABASE_URL:
        try:
            remember_telegram_chat_id(chat_id)
        except Exception:
            pass
    ...
    if not chat_id:
        return False
    if not DATABASE_URL:
        send_message(chat_id, "Task save nahi ho saka: DATABASE_URL configured nahi hai.")
        return True
    try:
        ensure_task_schema()
        task_text, due_at = parsed
        task_id = create_task(user_id=_get_user_id(message), chat_id=chat_id, task_text=task_text, due_at=due_at, timezone=DEFAULT_TIMEZONE)
        due_text = _format_local_time(due_at, DEFAULT_TIMEZONE)
        send_message(chat_id, f"✅ Task saved successfully.\n\nTask ID: #{task_id}\nTime: {due_text} IST\nAction: {task_text}\n\nUsi time Telegram par reminder bhejunga aur turant execution report bhi dunga.")
        return True
    except Exception as exc:
        send_message(chat_id, f"Task save karte waqt error aaya: {str(exc)[:900]}")
        return True
...
def _execute_task(task: dict) -> None:
    ...
    try:
        ...
        send_message(chat_id, f"⏰ Scheduled Task Reminder\n\nTask #{task_id}\n{task_text}\n\nScheduled time: {due_text}")
        report_time = datetime.now(ZoneInfo(timezone_name)).strftime("%I:%M:%S %p").lstrip("0")
        report = f"Task #{task_id} executed successfully.\nReport time: {report_time} {timezone_name}\nStatus: Delivered on schedule."
        send_message(chat_id, f"✅ Execution Report\n\n{report}")
        complete_task(task_id, report)
    except Exception as exc:
        retry_task(task_id, str(exc)[:1200], max_attempts=MAX_TASK_ATTEMPTS)
