...
class AgentCore:
    ...
    def run(self, query: str, context: Optional[Dict[str, Any]] = None) -> ExecutionResult:
        ...
        # Multi-agent/deep research is now explicit-only. Normal research is handled once by ai.agent.
        ...
        try:
            knowledge_context = build_knowledge_context(user_id=user_id, query=query, limit=8) if user_id is not None else ""
        ...
        # Do NOT search here. ai.agent performs exactly one research pass, avoiding duplicate web calls.
        web_context = ""
        current_time_utc = datetime.now(timezone.utc)
        current_time_local = datetime.now().astimezone()
        prompt = self._build_prompt(query, plan, memory_context, knowledge_context, universal_context, graph_context, file_context, web_context, recent_messages, routed_provider, current_time_utc, current_time_local, location_context)
    ...
    def _result_from_specialist_run(self, query, plan, result, user_id, mode):
        ...
        run_id = None
        if self.intelligence_telemetry_enabled:
            try: run_id = record_run(user_id=user_id, query=query, answer=answer, provider=provider, model=model, skill=plan.primary_skill, confidence=0.85, verification_status="multi_agent_reviewed", metadata={"orchestration_mode": mode, "agent_count": result.get("agent_count", 0), "research_source_count": result.get("source_count", 0)})
            except Exception: pass
        metadata = {"agent_version": self.version, "primary_skill": plan.primary_skill, "model": model, "orchestration_mode": mode, "agent_count": result.get("agent_count", 0), "research_source_count": result.get("source_count", 0), "research_sources": result.get("research_sources", []), "intelligence_run_id": run_id, "model_routing_reason": result.get("routing_reason", "")}
        return ExecutionResult(answer=answer, success=True, provider=provider, skill=plan.primary_skill, metadata=metadata)

    @staticmethod
    def _should_use_multi_agent(query, plan) -> bool:
        text = str(query or "").lower()
        explicit = ("deep research", "multi agent", "multi-agent", "debate", "peer review", "research panel", "systematic review")
        return any(m in text for m in explicit)

    @staticmethod
    def _should_deep_research(query, plan) -> bool:
        text = str(query or "").lower()
        markers = ("deep research", "research paper", "academic", "scientific literature", "systematic review", "compare studies", "citation required", "source comparison")
        return any(m in text for m in markers)

    @staticmethod
    def _needs_rich_context(query, plan) -> bool:
        text = str(query or "").lower()
        markers = ("compare", "architecture", "strategy", "analysis", "analyze", "research", "design", "debug", "audit", "evaluate", "explain why", "how does", "pros and cons")
    ...
    def _build_web_context(self, query: str, plan, location=None) -> str:
        # Kept for compatibility with callers; actual research now happens once in ai.agent.
        return ""
